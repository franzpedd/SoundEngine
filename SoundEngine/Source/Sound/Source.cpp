...
namespace Cosmos::sound
{
		...
		: mPath(path)
	{
		...
		{
			std::cout << "The bit depth " << audioFile.GetBitDepth() << " is not supported by OpenAL. Use 8 or 16" << std::endl;
			return;
		}
	...
	void Source::SetPosition(const float& x, const float& y, const float& z)
	{
		if (mStereo)
		{
			std::cout << "Stereo audios Spatialization is not supported by OpenAL" << std::endl;
			return;
		}
